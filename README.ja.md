...
322行目、468行目に横置き＆縦置きそれぞれの透過処理設定があります。

```hsp3 L322/L468
SetLayeredWindowAttributes _tb_mhtask, 0xffffff, 200, 0x2
```

現在は 末尾の dwFlags が 0x2 、 bAlpha=200（約78%不透明）で半透明になっています。  
タスクバーに埋め込まれるコンテナウインドウの背景は白 (0xffffff) ですので、末尾の dwFlags を 0x1 に書き換えると白を透明化することになり、背景を透過させて埋め込むことが可能になります。
